...
public class Sphere extends RadialGeometry {
	...
	public List<Point3D> findIntersections(Ray _ray) {
		...
		if (rayPnt.equals(sphereCenter)) {
			sphereLst.add(0, sphereCenter.addVec(rayVec.scale(_radius)));
			return sphereLst;
		}
		...
		double rPPVDist = sphereCenter.distance(rPntPVec);// check distance (rayPnt+rayVec) to sphere center to check if
		...
		if (rPPVDist < disP_O) {
			...
			if (disP_O > _radius) {
				if (d > _radius) {
					return sphereLst;
				} else if (d == _radius) {// then the ray tangent to the sphere
					p1 = rayPnt.addVec(rayVec.scale(_tm));
					// sphereLst.add(0, p1);
					// return sphereLst;
				} else {
					double th = Math.sqrt((Math.pow(_radius, 2) - Math.pow(d, 2)));// distance from first ray<->sphere
				...
				Vector revP_O = sphereCenter.subtract(revRayPnt);
				...
				double th = Math.sqrt((Math.pow(_radius, 2) - Math.pow(revD, 2)));
